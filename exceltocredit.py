...
os.makedirs('exceljson', exist_ok=True)
...
for idx, row in df.iterrows():
    ...
    json_data = {
        ...
        "ExemptionNumber": "",
        "TotalInvoiceAmount": str(row['AMOUNT(116)']).replace(',', ''),
        "TotalTaxableAmount": str(row['AMOUNT(100)']).replace(',', ''),
        "TotalTaxAmount": str(row['AMOUNT(16)']).replace(',', ''),
        "ItemDetails": [{
            "HSCode": "",
            "HSDesc": "DEP 1",
            "Category": "",
            "UnitPrice": str(row['AMOUNT(100)']).replace(',', ''),
            "Quantity": "1.00",
            "ItemAmount": str(row['AMOUNT(100)']).replace(',', ''),
            "TaxRate": "16.00",
            "TaxAmount": str(row['AMOUNT(16)']).replace(',', '')
        }]
    }

    filename = f"exceljson/{middleware_number}.txt"
