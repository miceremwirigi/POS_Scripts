...
def main():
    ...
    print(f"Found {len(rows)} job cards. Filtering for the date range {start_date_str} to {end_date_str}...")
    ...
    if not jobcard_data:
        print(f"No job cards found for the date range {start_date_str} to {end_date_str}.")
        return
    ...
    # Save to Excel
    excel_file_name = f"jobcards_{start_date_str}_to_{end_date_str}.xlsx"
    df.to_excel(excel_file_name, index=False)
    
    # Save to PDF
    pdf_file_name = f"jobcards_{start_date_str}_to_{end_date_str}.pdf"
    report_title = f"Job Card Report from {start_date_str} to {end_date_str}"
