...
def process_backup_directory(backup_dir_path):
    ...
    if not inv_path or not eod_path:
        ...
        return receipts_by_date, None

    print("Processing all invoice files to gather data...")
    ...
    return receipts_by_date, eod_path

def generate_and_save_eods(receipts_by_date, eod_folder_path):
    """
    Generates a new, complete, and chronologically ordered set of EOD reports
    and saves them in the correct folder structure based on a reference point.
    """
    ...
    # --- Logic for EOD alignment based on specific reference date ---
    reference_date_str = "2025-06-19"
    target_eod_number = 962
    
    # 1. Find the date span of the invoices
    first_invoice_date = datetime.strptime(all_invoice_dates[0], '%Y-%m-%d')
    
    # 2. Find the reference date within the invoice dates
    try:
        reference_date_obj = datetime.strptime(reference_date_str, '%Y-%m-%d')
        if reference_date_obj not in [datetime.strptime(d, '%Y-%m-%d') for d in all_invoice_dates]:
            print(f"Error: The reference date {reference_date_str} has no invoices.")
            return
    except ValueError:
        print(f"Error: Invalid date format for reference date: {reference_date_str}")
        return
        
    # 3. Calculate the number of days from the first invoice to the reference date
    days_span_to_reference = (reference_date_obj - first_invoice_date).days
    
    # 4. Calculate the number of zero-EODs needed at the beginning
    start_offset_days = target_eod_number - (days_span_to_reference + 1)
    
    if start_offset_days < 0:
        print(f"Error: Cannot achieve target EOD number. The reference date is too late in the timeline.")
        return
        
    # Determine the final date range
    start_date = first_invoice_date - timedelta(days=start_offset_days)
    end_date = datetime.strptime(all_invoice_dates[-1], '%Y-%m-%d')
    date_range = [start_date + timedelta(days=x) for x in range((end_date - start_date).days + 1)]
    
    print(f"Generating a total of {len(date_range)} EOD reports.")
...
def main():
    ...
    receipts_by_date, eod_path = process_backup_directory(backup_dir_path)

    if not receipts_by_date:
        print("Could not find sufficient data to generate EOD reports.")
        return
    ...
    generate_and_save_eods(receipts_by_date, eod_path)
